...
def _build_line_to_location_map(
    ...
    #133 — the v3.30.20 ``skipped_deck_conflicts`` third element was removed:
    it reported deck auto-creates dropped by the pre-v3.1.0 legacy decks.name
    UNIQUE auto-index (dead since the v4 cutover). The only other reason a
    deck-create is dropped — an orphaned Deck with no paired StorageLocation
    (rare pre-v3.10.6 drift) — still falls through to Pending, now without a
    (formerly mislabeled "legacy conflict") result-page notice.
    """
